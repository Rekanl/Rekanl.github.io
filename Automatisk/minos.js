 ...
 */
  ...
  * @example  html='<div>{x}</div>', js='let x=1;interpolate();  mind({x});'
  * html changed to <div><span></span></div>
  * changes to x will be updated into span.innerHTML
  * NOTE: start state not set, change x to set span.innerHTML
  */
  ...
  */
 export function mind(obj) {
     return new Proxy(obj, {
         set: function (target, property, value) {
             const v = Number.isFinite(value) ? value.toFixed(2) : value;
